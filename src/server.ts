...
export function buildServer() {
  ...
  app.addContentTypeParser('application/json', { parseAs: 'buffer' }, (req, body, done) => {
    ...
    try {
      ...
      // A SyntaxError carries no statusCode, so Fastify would answer 500 and
      // page the operator for what is really a malformed client request.
  ...
  app.get('/tenders', async (req, reply) => {
    ...
    // parseInt('9'.repeat(400)) is Infinity, which reached SQLite as an OFFSET
    // and threw — a 500 that any crawler following a mangled link could trigger.
    const MAX_PAGE = 2000;
    const parsedPage = Number.parseInt(q.page ?? '1', 10);
    const page = Number.isFinite(parsedPage) ? Math.min(MAX_PAGE, Math.max(1, parsedPage)) : 1;
    const limit = 25;
