...
import {
  ...
  isValidEmail, markConfirmationSent, subscriberStats, unsubscribe, updateProfile,
} from './core/subscribers.js';
import { sendConfirmationEmail, sendSettingsLinkEmail, sendWelcomeEmail } from './core/emails.js';
import { adminPage } from './web/admin.js';
...
export function buildServer() {
  ...
  // Brand assets (logo, OG image). Cached hard — they never change per deploy.
  // Registered only when present so a slim deployment still boots.
  ...
  app.post('/subscribe', async (req, reply) => {
    ...
    if ((b.website ?? '').trim()) {
      return html(reply, layout({ title: 'Check your inbox', body: '<h1 style="margin-top:32px">Check your inbox.</h1>' }));
    }
    ...
    const body = `<h1 style="margin-top:32px">Check your inbox.</h1>
      ...
      Click it and your alerts start — that one click is what keeps this list spam-free and
      legally clean under EU rules.</p>
      <p style="color:#64748b;font-size:14px">Nothing arrives? Check spam, or
      <a href="/">try again</a>.</p>`;
    return html(reply, layout({ title: 'Confirm your subscription', body }));
  });

  app.get('/confirm', async (req, reply) => {
    const q = req.query as Record<string, string | undefined>;
    const claims = q.t ? verifyToken<{ sub: number; scope: string }>(q.t) : null;
    if (!claims?.sub || claims.scope !== 'confirm') {
      return html(reply.code(400), layout({
        title: 'Link expired',
        body: `<h1 style="margin-top:32px">That link has expired.</h1>
          <p class="lede">Confirmation links last 14 days. Sign up again and we'll send a fresh one.</p>
          <p><a class="btn" href="/">Back to signup</a></p>`,
      }));
    }
    const sub = confirmSubscriber(claims.sub);
    if (!sub) return reply.code(404).send({ error: 'not found' });
    logEvent('subscriber.confirmed', { id: sub.id });
    try {
      await sendWelcomeEmail(sub.id, sub.email, { pro: sub.plan === 'pro' });
    } catch (err) {
      req.log.error(err, 'welcome email failed');
    }
    const token = signToken({ sub: sub.id, scope: 'account' }, 30);
    const body = `<h1 style="margin-top:32px">Confirmed. You're in.</h1>
      <p class="lede">Your first digest goes out on the next run. Two minutes tuning your filters now
      is the difference between a useful inbox and noise.</p>
      <p><a class="btn" href="/account?t=${encodeURIComponent(token)}">Set my filters →</a></p>
      <p style="margin-top:18px"><a href="/pricing">Or upgrade to daily alerts (${h(config.billing.priceLabel)})</a></p>`;
    return html(reply, layout({ title: 'Subscription confirmed', body }));
  });
  ...
  app.post('/account/link', async (req, reply) => {
    ...
    if (sub && sub.status !== 'unsubscribed') {
      try {
        await sendSettingsLinkEmail(sub.id, sub.email);
      } catch (err) {
        req.log.error(err, 'settings link email failed');
      }
    }
    logEvent('account.link.requested', { email });
    const body = `<h1 style="margin-top:32px">Check your inbox</h1>
  ...
  /** Private operator dashboard. Guarded by APP_SECRET in the query string. */
  app.get('/admin', async (req, reply) => {
    const q = req.query as Record<string, string | undefined>;
    if (q.key !== config.security.secret) {
      return reply.code(403).type('text/html').send(
        layout({ title: 'Forbidden', body: '<h1 style="margin-top:32px">403</h1><p>Append <code>?key=YOUR_APP_SECRET</code>.</p>' }),
      );
    }
    return html(reply, adminPage(q.key));
  });

  /** Job triggers from the admin dashboard buttons (form post, then redirect back). */
  app.post('/admin/run/:job', async (req, reply) => {
    const b = (req.body ?? {}) as Record<string, string>;
    if (b.key !== config.security.secret) return reply.code(403).send({ error: 'forbidden' });
    const { job } = req.params as { job: string };
    const runner = { ingest: runIngest, 'digest-daily': runDailyDigest, 'digest-weekly': runWeeklyDigest }[job];
    if (!runner) return reply.code(404).send({ error: 'unknown job' });
    await runner({});
    return reply.redirect(`/admin?key=${encodeURIComponent(b.key)}`, 303);
  });
