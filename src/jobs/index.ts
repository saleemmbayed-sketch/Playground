  ...
  return guarded('digest.weekly', async () => {
    ...
    for (const s of subs) {
      ...
      await sendDigestSafely(s, s.profile, pool, {
        ...
        period: 'this week',
        // Never mail an empty digest, free tier included. A weekly email with no
        // matches in it is pure upsell, and that is what earns spam complaints —
        // which cost us the sending reputation the whole product depends on.
        skipIfEmpty: true,
      }, stats);
    }
