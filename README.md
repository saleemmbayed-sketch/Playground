...
scripts/uat.mjs          98 black-box HTTP acceptance checks (run against any deployment)
scripts/uat-lifecycle.ts 50 stateful journey + failure-injection checks (throwaway DB)
```
...
## 6.2 Acceptance testing (the unhappy paths)

The unit suite proves the pieces work. Two UAT harnesses prove the *deployed
system* fails safely, because in an unattended business a 500 at 3am is a
silent outage and a duplicate email is a spam complaint.

```bash
npm test                 # 103 unit/integration tests
npm run uat              # 98 black-box HTTP checks against a running server
npm run uat:lifecycle    # 50 stateful journey + failure-injection checks
```

`scripts/uat.mjs` is black-box: it only speaks HTTP, so you can point it at
production (`BASE_URL=https://tenderping.eu npm run uat`) as a post-deploy gate.
It covers 404s, XSS/SQLi/traversal payloads, pagination edges, forged tokens,
paywall enumeration, rate limiting, webhook signature forgery, admin/ops
authorisation, HTTP method hygiene and SEO metadata.

`scripts/uat-lifecycle.ts` runs against a throwaway database and drives the
journeys end to end with real signed tokens — signup → confirm → digest →
unsubscribe, bounce → suppression, free → Pro → Edge → past_due → cancelled —
then breaks the two dependencies we do not control (TED and the mail provider)
and asserts the system degrades instead of dying.

**What the first full run actually found** — all four are fixed, each pinned by
a regression test in `test/http.test.ts`:

| Finding | Impact | Fix |
| --- | --- | --- |
| `/tenders?page=<400 digits>` → **500** | `parseInt` overflowed to `Infinity`, reaching SQLite as an OFFSET (`datatype mismatch`). Any crawler following a mangled link could trigger it. | Page clamped to a finite `1…2000`. |
| Malformed JSON body → **500** | The content-type parser threw a bare `SyntaxError`; Fastify reports that as a server error, so a bad client request would page the operator. | Parser tags the error `statusCode = 400`. |
| Empty weekly digest | Free subscribers were mailed even with zero fresh matches — a pure upsell email, the fastest way to earn spam complaints and lose sending reputation. | `skipIfEmpty` now applies to every tier. |
| Outbox path hardcoded | Tests and parallel environments read each other's mail, which masked a dedupe result. | New `MAIL_OUTBOX_DIR` setting. |

Two apparent failures turned out to be correct behaviour worth recording: the
signup rate limiter counts *rejected* submissions too (so a barrage of invalid
addresses correctly locks the bucket), and a subscriber with more matches than
fit in one digest legitimately receives the next batch on the next run. The
guarantee that matters — *no notice is ever emailed to the same person twice* —
is now asserted directly by diffing the links across every message sent to one
address.
